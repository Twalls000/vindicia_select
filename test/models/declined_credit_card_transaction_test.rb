...
class DeclinedCreditCardTransactionTest < ActiveSupport::TestCase
  ...
  class WorkFlow < DeclinedCreditCardTransactionTest
    ...
    test "workflow states" do
      assert_equal [:entry, :queued_to_send, :pending, :in_error], DeclinedCreditCardTransaction.aasm.states.map(&:name)
    end
    test "workflow events" do
      assert_equal [:queue_to_vindicia, :send_to_vindicia, :mark_in_error, :captured_funds, :failed_to_capture_funds], DeclinedCreditCardTransaction.aasm.events.map(&:name)
    end
    test "workflow permitted based on entry state" do
      assert_equal [:queue_to_vindicia, :mark_in_error], @trans.aasm.events(:permitted => true).map(&:name)
    end
